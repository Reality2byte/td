    ...
    if (content_text != nullptr) {
      ...
      *content_text = FormattedText();
      ...
      if (content_->get_type() == MessageContentType::Text) {
        auto content = get_message_content_object(content_.get(), td, DialogId(), MessageId(), DialogId(), false, true,
                                                  DialogId(), 0, false, true, -1, false, false);
...
td_api::object_ptr<td_api::messageReplyToMessage> RepliedMessageInfo::get_message_reply_to_message_object(
  ...
  if (content_ != nullptr) {
    content = get_message_content_object(content_.get(), td, DialogId(), message_id, DialogId(), false, true,
                                         DialogId(), 0, false, true, -1, false, false);
